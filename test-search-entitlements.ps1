<#
.SYNOPSIS
    Test script for Sentinel EMS Search Entitlements API

.DESCRIPTION
    This script tests the Search Entitlements endpoint of the Thales Sentinel EMS REST API.
    It retrieves all entitlements associated with a specific customer ID.

.EXAMPLE
    .\test-search-entitlements.ps1

.NOTES
    Version:        1.0.0
    Author:
    Creation Date:  2025-10-31
    Purpose:        Test EMS Search Entitlements API endpoint
    Reference:      .claude/skills/ems-api/skill.md - Search Entitlements
#>
...
function Initialize-ApiHeaders {
    ...
    try {
        ...
        # Set up headers per ems-api skill documentation
        $script:ApiHeaders = @{
...
function Invoke-SearchEntitlements {
    <#
    .SYNOPSIS
        Call the Search Entitlements API endpoint
    .DESCRIPTION
    ...
    try {
        ...
        $apiUrl = "$script:EmsBaseUrl/ems/api/v5/entitlements"

        # Add query parameter for customer ID
        $apiUrl += "?customerId=$script:CustomerId"
...
function Show-Response {
    ...
    if ($Response.entitlements) {
        ...
        Write-Host "No entitlements found for customer ID: $script:CustomerId" -ForegroundColor Yellow
    }
}
...
try {
    ...
    # Call the API
    $response = Invoke-SearchEntitlements
